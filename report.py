...
def build_styles():
    ...
    styles = {
        ...
        "h2": ParagraphStyle("H2", parent=base["Heading2"], fontSize=10,
                             textColor=colors.HexColor("#2d5a96"), spaceBefore=4, spaceAfter=3, leading=12),
...
def array_to_flowable(arr, max_width=8.5 * cm):
...
def generate_report(output_path, frame_annotated, features, calculi_info,
    ...
    table_meta = dense_table(meta_rows, col_widths=[4.5 * cm, 4.0 * cm])
    ...
    if classification is not None:
        ...
        header_table = Table([[table_meta, [diag_p, table_prob]]], colWidths=[9.0 * cm, 9.6 * cm])
    ...
    story.append(Paragraph("Visualización y Métricas Morfométricas / Textura", styles["h2"]))
    
    img_flowable = array_to_flowable(frame_annotated, max_width=8.5 * cm)
    ...
    table_metrics = dense_table(combined_metrics, col_widths=[2.8 * cm, 2.2 * cm, 2.8 * cm, 2.2 * cm])

    side_by_side = Table([[img_flowable, table_metrics]], colWidths=[8.8 * cm, 9.8 * cm])
    ...
    if features.get("has_calculi") == 1 and calculi_info:
        ...
        table_calc_summary = dense_table(summary_rows, col_widths=[4.5 * cm, 4.0 * cm])

        detail_rows = [["ID", "Diámetro (mm)", "Área (px)"]]
        for c in calculi_info[:4]:
            detail_rows.append([f"C{c['id']}", fmt(c["diam_mm"], 2), str(c["area_px"])])
        table_calc_detail = dense_table(detail_rows, col_widths=[2.5 * cm, 3.5 * cm, 3.0 * cm], align_center=True)

        calc_side_by_side = Table([[table_calc_summary, table_calc_detail]], colWidths=[9.0 * cm, 9.6 * cm])
